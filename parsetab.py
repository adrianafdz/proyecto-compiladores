...
_lr_productions = [
  ...
  ('expresion -> STR f_string_expr','expresion',2,'p_expresion','aloop.py',821),
  ('f_string_expr -> <empty>','f_string_expr',0,'p_f_string_expr','aloop.py',824),
  ('f_expres -> <empty>','f_expres',0,'p_f_expres','aloop.py',829),
  ('exp -> term','exp',1,'p_exp','aloop.py',853),
  ('exp -> exp OPTERM f_oper term f_exp','exp',5,'p_exp','aloop.py',854),
  ('f_exp -> <empty>','f_exp',0,'p_f_exp','aloop.py',858),
  ('term -> fact','term',1,'p_term','aloop.py',882),
  ('term -> term OPFACT f_oper fact f_term','term',5,'p_term','aloop.py',883),
  ('f_term -> <empty>','f_term',0,'p_f_term','aloop.py',887),
  ('f_oper -> <empty>','f_oper',0,'p_f_oper','aloop.py',912),
  ('fact -> ( lparen expresion ) rparen','fact',5,'p_fact','aloop.py',916),
  ('fact -> var','fact',1,'p_fact','aloop.py',917),
  ('fact -> NUM f_fact','fact',2,'p_fact','aloop.py',918),
  ('fact -> OPTERM NUM','fact',2,'p_fact','aloop.py',919),
  ('fact -> CALL func f_return_val f_end_call f_end_check','fact',5,'p_fact','aloop.py',920),
  ('lparen -> <empty>','lparen',0,'p_lparen','aloop.py',933),
  ('rparen -> <empty>','rparen',0,'p_rparen','aloop.py',937),
  ('f_fact -> <empty>','f_fact',0,'p_f_fact','aloop.py',941),
  ('f_return_val -> <empty>','f_return_val',0,'p_f_return_val','aloop.py',947),
  ('condicion -> IF ( expresion ) f_if THEN { estatutos } condicionp f_endif','condicion',11,'p_condicion','aloop.py',973),
  ('condicionp -> ELSE f_else { estatutos }','condicionp',5,'p_condicionp','aloop.py',976),
  ('condicionp -> empty','condicionp',1,'p_condicionp','aloop.py',977),
  ('f_if -> <empty>','f_if',0,'p_f_if','aloop.py',980),
  ('f_endif -> <empty>','f_endif',0,'p_f_endif','aloop.py',990),
  ('f_else -> <empty>','f_else',0,'p_f_else','aloop.py',995),
  ('while -> WHILE f_while ( expresion f_exprwhile ) DO { estatutos } f_endwhile','while',11,'p_while','aloop.py',1002),
  ('f_while -> <empty>','f_while',0,'p_f_while','aloop.py',1005),
  ('f_exprwhile -> <empty>','f_exprwhile',0,'p_f_exprwhile','aloop.py',1009),
  ('f_endwhile -> <empty>','f_endwhile',0,'p_f_endwhile','aloop.py',1019),
  ('for -> FOR expresion f_for_start TO expresion f_for_to { estatutos } f_for_end','for',10,'p_for','aloop.py',1026),
  ('f_for_start -> <empty>','f_for_start',0,'p_f_for_start','aloop.py',1029),
  ('f_for_to -> <empty>','f_for_to',0,'p_f_for_to','aloop.py',1039),
  ('f_for_end -> <empty>','f_for_end',0,'p_f_for_end','aloop.py',1055),
  ('to_num -> TO_NUMBER ( STR )','to_num',4,'p_to_num','aloop.py',1067),
  ('to_num -> TO_NUMBER ( var )','to_num',4,'p_to_num','aloop.py',1068),
  ('to_str -> TO_STRING ( expresion )','to_str',4,'p_to_str','aloop.py',1081),
  ('input -> INPUT ( var )','input',4,'p_input','aloop.py',1094),
  ('write -> PRINT ( write_list )','write',4,'p_write','aloop.py',1098),
  ('write_list -> write_list & write_listp','write_list',3,'p_write_list','aloop.py',1101),
  ('write_list -> write_listp','write_list',1,'p_write_list','aloop.py',1102),
  ('write_listp -> STR f_string','write_listp',2,'p_write_listp','aloop.py',1105),
  ('write_listp -> var','write_listp',1,'p_write_listp','aloop.py',1106),
  ('write_listp -> CALL to_str','write_listp',2,'p_write_listp','aloop.py',1107),
  ('f_string -> <empty>','f_string',0,'p_f_string','aloop.py',1114),
  ('return -> RET ( expresion )','return',4,'p_return','aloop.py',1120),
  ('empty -> <empty>','empty',0,'p_empty','aloop.py',1152),
]
